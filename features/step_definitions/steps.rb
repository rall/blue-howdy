...
When(/I run 'ujust howdy-pam-add' (to|but don't) add howdy to (login|sudo)/) do |act, pam|
  ...
  run_command(container.exec_cmd("ujust howdy-pam-add", interactive: true, user: 0, debug: true))
...
Then(/I can run 'ujust (.*)'/) do |just_task|
  run_command(container.exec_cmd("ujust #{just_task}", interactive: true, user: 0), fail_on_error: true, exit_timeout: 10)
end
